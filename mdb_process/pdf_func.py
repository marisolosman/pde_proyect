...
import sys
sys.path.append('c:/felix/ora/python_scripts/pde_proyect/mdb_process/')
...
def save_tabla_percentil(in_di, matdata):
    ...
    f_name = './percentiles_' + in_di['type'] + '_' + in_di['var'] + '.txt'
    tabla_percentil.apply(pd.to_numeric, errors='ignore')
    tabla_percentil.to_csv(f_name, sep=';', decimal=',', float_format='%.3f', index=False)
    ...
    if all(llaves in in_di for llaves in ['outfo', 'estacion', 'var']):
        ...
        for mes in range(1, 13):
            if mes - 1 <= 0:
                ...
                cnd = [mes - 1, mes, mes + 1]
            datos = df[df['month'].isin(cnd)].ens_mean.to_numpy()
            if in_di['var'] == 'precip':
                datos[datos < 1.] = np.nan
            #
            prc = calc_percentil(datos)
            fmat[:, mes] = prc
...
def calc_pdf_OBS(in_di):
    ...
    for mes in range(1, 13):
        if mes - 1 <= 0:
            ...
            cnd = [mes - 1, mes, mes + 1]
        print(df.columns)
        datos = df[df['month'].isin(cnd)].variable.to_numpy()
        if in_di['var'] == 'precip':
            datos[datos < .1] = np.nan
        prc = calc_percentil(datos)
        fmat[:, mes] = prc
