...
exc_file = 'balance_RESIS_FL40-45_S1-VII_NORTE.xls'
...
#                   alpha=0.3,
#                   facecolor='limegreen')
ax[0].boxplot(c.ALMR, 1)
# prono: media del ensamble
#ax[0].plot(c.dtimes, np.mean(c.ALMR,axis=1), color='yellow', linewidth=2)
# observado
ax[0].plot(c.almr_obs,'k',linewidth=2)
ax[0].set_ylim(0, 120)
...
ax[1].boxplot(np.transpose(d.ALMR), 0, positions=np.arange(np.shape(d.ALMR)[0]))
...
plt.savefig('BH_ic_' + fecha + '_boxplot.png')
