...
def get_data_from_grib(grb_file, xlat, xlon, fecha):
    ...
    ye   = xlat
    tiempos = grbs.valid_time.values <= np.datetime64(fecha)
...
fmat = np.empty([len(fval), 4, 31])
for idx, fecha in enumerate(fval):
    f_fecha = fecha + dt.timedelta(days=32)
    dic['idate'] = fecha
    dic['fdate'] = f_fecha
            ...
            if 'vacio' in [f_ps, f_q2, f_t2]:
                ...
                try:
                    ...
                    if operacion == 'mean':
                        ...
                        fvar = 'hrmean'
                        resu = d_hr.resample(rule='1D').mean()
                    sel_d = np.logical_and(resu.index >= arg_tz.localize(fecha),
                                           resu.index <= arg_tz.localize(fecha +
        ...
        for arch in archi:
            if arch == 'vacio':
                ...
                try:
                    ...
                    ye   = dic['lat_e']
                    tiempos = grbs.valid_time.values <= np.datetime64(dic['fdate'])
                    data = grbs[nvar].sel(longitude=xe, latitude=ye, step=tiempos, method='nearest')
...
#                                 date_format='%Y-%m-%d',index_label='fecha', header=[fvar])
            ...
            ens += 1
for i in range(31):
    df = create_summary_file(dic, fval, fmat[:, :, i], i)
    #print(df)
    sel_col = list(df)[1::]
    df[sel_col] = df[sel_col].apply(pd.to_numeric, errors='ignore')
    n_file = cpta_salida + 'data_final_' + fvar + '_' + "{:02d}".format(i) + '.txt' 
